...
上手三剑客（grep,sed,awk）之前，需要熟悉正则表达式。
...
sed是以数据流方式读取文件内容，每次会读取一行进行匹配，看是否满足匹配要求，满足则做增删改查操作，操作完成后屏幕打印，打印出来后重复刚才的动作读取下一行进行匹配，以此循环。

| 参数 | 说明 |
| - | - |
| -n | 取消自动打印,通常与p搭配（可以死记np组合） |
| p | 打印 |
| s | 替换 |
| d | 删除 |
| -r | 正则 |
| cai | 替换replace，追加append，插入insert |
| s | 替换，g 为全局替换。替换为正则表达式，需要加参数-r |
...
| - | - |
| NR | 行号   NR==2：第二行 |
| NF | 字段数量（多少列）  $NF：最后一个字段 |
| -F | 输入字段分隔符，默认空格。-F: 等同于-v FS=:    ----说明：-v 是修改awk变量 |
| OFS | 输出字段分隔符，默认空格。|
