/*  Tiger Liu: 2022-09-27
   for plsql developer : Command Window , collect sqlhc
   put plsqlhc.sql to Directory set in plsql developer "directory" "Command File"
   privilege : grant connect / select any dictionary / SELECT_CATALOG_ROLE / execute on DBMS_WORKLOAD_REPOSITORY to xxx;
   usage: 
   New "command window"
   SQL>@plsql_sqlhc  xxxxxxxxxxxxx   (xxxxxxxxxxxxx is a sql_id)
*/
rem SPO sqlhc.log
SET DEF ^ TERM OFF ECHO ON AUTOP OFF VER OFF SERVEROUT ON SIZE 1000000;
set colwidth 8000

undefine input_sql_id
undefine sql_id

REM
DEF health_checks = 'Y';
DEF shared_cursor = 'N';
DEF sql_monitor_reports = '1';
REM
DEF script = 'sqlhc';
DEF method = 'SQLHC';
...
PRO INPUT SQL_ID : "^^input_sql_id."
PRO
--SET TERM OFF;

COL sql_id NEW_V sql_id FOR A13;
--tiger: Adjust for plsql developer Command Window
select 
 nvl(
    (SELECT sql_id
      FROM gv$sqlarea
...
--select 'input sql_id: '||'^^sql_id.' from dual;
        
--SET TERM ON;
WHENEVER SQLERROR EXIT SQL.SQLCODE;

BEGIN
  IF '^^sql_id.' ='NOT_FOUND' THEN
    --IF :license IN ('T', 'D') THEN
      RAISE_APPLICATION_ERROR(-20200, 'INPUT SQL_ID "^^input_sql_id." not found in memory nor in AWR.');
    --ELSE
...
SELECT :E_GLOBAL, 'OFFLOAD', SYSTIMESTAMP, 'OFFLOAD OFF',
  ...
  FROM v$system_parameter2 
 WHERE  UPPER(name) = 'SHARED_SERVERS'
   AND UPPER(value) > 0
   AND '^^exadata.' = 'Y';   
...
SELECT :E_GLOBAL, 'OFFLOAD', SYSTIMESTAMP, 'OFFLOAD OFF',
  ...
  FROM v$sql 
 WHERE  TRIM(UPPER(SUBSTR(LTRIM(sql_text),1,6))) IN ('INSERT','UPDATE','DELETE','MERGE')
   AND sql_id = '^^sql_id.'
   AND ROWNUM = 1
   AND '^^exadata.' = 'Y';  
...
DECLARE
  l_sql_text varchar2(32767) := :sql_text;
  l_pos NUMBER;
BEGIN
  WHILE NVL(LENGTH(l_sql_text), 0) > 0 and LENGTH(l_sql_text)<32767
  LOOP
 ...
 *
 * ------------------------- */
--PRO <a name="spm"></a><h2>SQL Plan Baselines (DBA_SQL_PLAN_BASELINES): not collect</h2>
--PRO
 ...
 *
 * ------------------------- */
--PRO <a name="prof"></a><h2>SQL Profiles (DBA_SQL_PROFILES): not collect</h2>
--PRO
 ...
 *
 * ------------------------- */
--PRO <a name="patch"></a><h2>SQL Patches (DBA_SQL_PATCHES): not collect</h2>
--PRO
...
SELECT /* ^^script..sql Tables */
  ...
  FROM (
WITH object AS (
SELECT /*+ MATERIALIZE */  ---tiger:  if  ora-12850 , replace with no_merge
       object_owner owner, object_name name
  FROM gv$sql_plan
 WHERE inst_id IN (SELECT inst_id FROM gv$instance)
   AND sql_id = '^^sql_id.'
...
set echo off linesize 8000 pages 8000 long 20000000 longchunksize 20000000 trimout on  trims on feedback off 
SPO ^^files_prefix._4_sql_monitor.html;
...
spool ^^files_prefix._5_sql_text.sql
...
declare
  ...
  v_len number;
  v_cnt number;
  l_sql_text_part varchar2(32767);
BEGIN 
  l_sql_id := '^^sql_id.';
  ...
  if l_sql_text='NOT_IN_AWR' then
    ...
    --for SQL length>32767, double while loop , tiger liu : 20211027 shanghai
    v_len:=dbms_lob.getlength(l_sql_text);
    v_offset:=1;
    v_cnt:=0;
    
    
    while v_len-v_offset>=0 loop
     --dbms_output.put_line(substr(o,v_offset,32767));
      l_sql_text_part:=substr(l_sql_text,v_offset,32767);
      WHILE NVL(LENGTH(l_sql_text_part), 0) > 0
          LOOP
            l_pos := INSTR(l_sql_text_part, CHR(10));
            --DBMS_OUTPUT.PUT_LINE('position:'||l_pos);
            IF l_pos > 0 THEN
              DBMS_OUTPUT.PUT_LINE(SUBSTR(l_sql_text_part, 1, l_pos - 1));
              l_sql_text_part := SUBSTR(l_sql_text_part, l_pos + 1);
            ELSE
              DBMS_OUTPUT.PUT_LINE(l_sql_text_part);
              l_sql_text_part := NULL;
            END IF;
          END LOOP;
     v_cnt:=v_cnt+1;
     v_offset:=32767*v_cnt+1;         
    end loop;
...
/* 
pro ==============if not both >0 then  some info may not be collected=====================
select 'gv$sql or dba_hist_sqlstat='||count(*)
from
(
...
select 'gv$sql_plan or dba_hist_sql_plan='||count(*)
from
(
...
host zip -m ^^files_prefix..zip ^^files_prefix.*

Pro
pro
Pro
pro =======================================================================================
Pro Task complete , ^^files_prefix..zip  generated in current directory.
pro =======================================================================================
pro
pro
pro
...
set serveroutput off def '&' head on
undef sql_id
undef input_sql_id
undef 1
